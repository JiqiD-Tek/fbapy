...
from backend.common._dataclasses import TokenPayload
...
async def get_terminal_user(user_id: int) -> GetUserInfoDetail:
    ...
    :return:
    """
    user_key = f'{settings.JWT_USER_REDIS_PREFIX}:terminal:{user_id}'
    cache_user = await redis_client.get(user_key)
    if not cache_user:
        async with async_db_session() as db:
...
async def jwt_authentication(token: str) -> GetUserInfoWithRelationDetail:
    ...
    extra_info = await redis_client.get(f'{settings.TOKEN_EXTRA_INFO_REDIS_PREFIX}:{ctx.user_id}:{token_payload.session_uuid}')
