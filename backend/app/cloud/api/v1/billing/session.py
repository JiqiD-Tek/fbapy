...
) -> ResponseSchemaModel[BillOpenSessionResult]:
    data = await billing_service.open_session(db=db, obj=obj, auth_did=auth_ctx.did)
...
async def close_session(
    ...
    auth_ctx: DeviceAuthParam = DependsDeviceAuth,
) -> ResponseSchemaModel[BillCloseSessionResult]:
    data = await billing_service.close_session(db=db, obj=obj, auth_did=auth_ctx.did)
    return response_base.success(data=data)
