...
async def text_to_speech(
    ...
    """http TTS"""
    if not text:
        raise KeyError('text is empty')

    url = await speech_service.text_to_speech(uid=f'{uuid.uuid4()}', text=text, retain=retain)
    return response_base.success(data=url)


@router.websocket('/transcriptions')
async def transcriptions(websocket: WebSocket) -> None:
    """websocket ASR"""
