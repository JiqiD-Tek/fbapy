...
class ChatService(CozeService):
    ...
    async def on_chat_update(self, uid: str, event: ChatUpdateEvent) -> None:
        """配置更新"""
    ...
    async def on_conversation_chat_cancel(self, uid: str, event: ConversationChatCancelEvent) -> None:
        """对话取消"""
        if not (channel := await channel_pool.get_channel(uid)):
            return
    ...
    async def on_conversation_message_create(self, uid: str, event: ConversationMessageCreateEvent) -> None:
        """对话消息"""
    ...
    async def _register_speech_callback(self, uid: str) -> None:
        ...
        async def on_finish_text(text: str) -> None:
            """asr识别完成回调"""
            await channel.put_nowait(
                ConversationAudioTranscriptCompletedEvent.model_validate({
        ...
        async def on_audio(delta: bytes | None) -> None:
            ...
            if delta == b'':  # 大模型处理完成
                ...
                await channel.put_nowait(
                    ConversationChatCompletedEvent.model_validate({
            ...
            await channel.put_nowait(
                ConversationAudioDeltaEvent.model_validate({'data': Message.build_assistant_audio(delta)})
            )  # 语音中
    ...
    async def _chatgpt_query(self, uid, text) -> None:
        ...
        await channel.put_nowait(
            ConversationAudioUrlEvent.model_validate({
                'data': ConversationAudioUrlEvent.Data.model_validate({'content': f'{channel.uid}.{tts_req_id}'})
            })
        )  # token.uuid.tts_req_id

        async def on_token(token_text: str) -> None:
            """大模型生成流式内容"""
            await channel.put_nowait(
                ConversationMessageDeltaEvent.model_validate({'data': Message.build_assistant_answer(token_text)})
            )  # 流式文本

        async def on_finish(final_text: str) -> None:
            """大模型完整流式内容"""
            await channel.put_nowait(
                ConversationMessageCompletedEvent.model_validate({'data': Message.build_assistant_answer(final_text)})
            )  # 完整文本
