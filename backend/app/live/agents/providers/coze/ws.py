...
class AsyncWebSocketClient:
    ...
    def __init__(self, *, url: str, token: str, **kwargs) -> None:
    ...
    async def ensure_connection(self, max_retries: int = 3, initial_delay: float = 1.0, timeout: float = 10.0) -> None:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    log.warning(f'连接尝试 {attempt}/{max_retries} 失败: {e!s}. {delay:.1f}秒后重试...')
                    await asyncio.sleep(delay)

        # 所有重试失败后的处理
        error_msg = f'无法建立连接 (尝试 {max_retries} 次). 最后错误: {last_exception!s}'
        log.error(error_msg)
    ...
    async def connect(self, timeout: float = 10.0) -> None:
        ...
        try:
            ...
            error_msg = f'无效的WebSocket地址: {self._url} ({e!s})'
            log.error(error_msg)
            raise ConnectionError(error_msg)

        except websockets.WebSocketException as e:
            error_msg = f'WebSocket连接错误: {e!s}'
            log.error(error_msg)
            raise ConnectionError(error_msg)

        except Exception as e:
            error_msg = f'未知连接错误: {e!s}'
            log.error(error_msg)
            raise ConnectionError(error_msg)
    ...
    async def _send(self, data: Any, timeout: float = 10.0) -> None:
        ...
        try:
            ...
            log.error(f'发送数据失败: {type(e).__name__}: {e!s}')
    ...
    async def aclose(self, code: int = 1000, reason: str = '无') -> None:
        ...
        try:
            ...
            log.error(f'关闭连接时发生意外错误: {type(e).__name__}: {e!s}')
