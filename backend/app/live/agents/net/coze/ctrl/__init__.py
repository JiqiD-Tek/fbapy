...
from backend.app.live.agents.net.coze.models import CozeModel, Message, WebsocketsEvent, WebsocketsEventType
...
class Command(CozeModel):
    ...
    class Payload(BaseModel):
        ...
        params: dict[str, Any] | None = Field(description='动态参数', examples=[{'volume': 80}])

    protocol: Literal['v1', 'v2'] = Field(
        default='v1',  # 显式设置默认值
        description='协议版本，默认为v1',
    )
    timestamp: int = Field(default_factory=lambda: int(datetime.now().timestamp()), description='事件创建时间戳（秒）')
    type: CommandType = Field(..., description='命令分类')
    payload: Payload

    @staticmethod
    def build_command(type: CommandType, cmd: str, params: dict[str, Any] | None = None) -> 'Command':
        return Command(type=type, payload=Command.Payload(cmd=cmd, params=params))
