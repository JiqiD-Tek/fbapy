...
from typing import AsyncGenerator, Final, Optional, Union, Dict, AsyncIterator
    ...
    async def _monitor_channels(self) -> None:
        ...
        while not self._is_closed:
            try:
                ...
                now = time.monotonic()
                inactive = [
                    conn.uid async for conn in self.iter_channels()
                ...
                log.info(
                    f"连接池状态监控 [当前连接数：{self.current_channels} "
                    f"| 最大连接容量：{self._capacity}]"
                )
